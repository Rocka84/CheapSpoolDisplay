...
CheapSpoolDisplay is a firmware for the ESP32 Cheap Yellow Display (CYD) supporting **multiple NFC standards** (OpenSpool, OpenTag3D, OpenPrintTag). It allows you to scan, view, and organize your 3D printer filament spools using standardized NFC formats.
...
- **Multi-Protocol NFC Support**: 
  - **ISO14443A** (NTAG215/216): Reads and writes **OpenSpool (JSON)**, **OpenTag3D (Binary)**, and **OpenPrintTag (CBOR)** formats using MFRC522.
...
- `set tag_format <openspool|opentag3d|openprinttag|ask>` (Default tag format selection)
